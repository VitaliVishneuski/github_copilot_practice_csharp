...
### Task 1: Write Code
1. Open `AdvancedOperations.cs` and implement the methods using GitHub Copilot assistance:
    - `FindMax`
    - `ReverseString`
    - `FormatDate`
    - `Factorial`
...
2. Open `AdvancedOperationsTest.cs` and write unit tests for methods above, making sure the coverage is acceptable.
...
1. Review the `CalculateStatistics` method in `AdvancedOperations.cs` that calculates statistics for a set of numbers but utilizes bad practices and violates DRY, KISS, SOLID, and other principles.
